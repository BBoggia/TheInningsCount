...
class s810DataEntryViewController: UIViewController, UITextFieldDelegate {
    ...
    func sendData() {
        ...
        FIRDatabase.database().reference().child("Database").child("softball810").child(teamName).child("\(self.timeStamp)").setValue("Player Number: \(playerNumberTextField.text!) Innings Pitched: \(pitchCountTextField.text!)")
    }

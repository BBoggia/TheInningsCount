//
//  s1314DataEntryViewController.swift
//  The Umpire
//
...
class s1314DataEntryViewController: UIViewController, UITextFieldDelegate {
    
    let ref = FIRDatabase.database().reference().child("Database/softball1314/")
    ...
    func sendData() {
        ...
        FIRDatabase.database().reference().child("Database").child("softball1314").child(teamName).child("\(self.databaseDate)").setValue("\(userDate) | Player#: \(playerNumberTextField.text!) | Innings Pitched: \(pitchCountTextField.text!)")
    }
